...
class AdapterPrint (private val items4 : MutableList<Item>, private val context: Context) : RecyclerView.Adapter<AdapterPrint.ItemsViewHolder>(){
    ...
    private fun saveItemToOtherSharedPref(item: Item) {
        ...
        editor?.apply()
        Toast.makeText(context, "${item.text1}  به بخش تحویل منتقل شد", Toast.LENGTH_SHORT)
            .show()
    }
    private fun removeItemFromSharedPreferences(item: Item) {
        val sharedPreferences = context.getSharedPreferences("Print", 0)
