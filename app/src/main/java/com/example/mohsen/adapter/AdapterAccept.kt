...
class AdapterAccept (private val items2 : MutableList<Item>, private val context: Context)
    : RecyclerView.Adapter<AdapterAccept.ItemsViewHolder>(){
    ...
    private fun saveItemToOtherSharedPref(item: Item) {
        ...
        editor?.apply()
        Toast.makeText(context, "${item.text1}  به بخش واریز منتقل شد", Toast.LENGTH_SHORT)
            .show()
    }
