...
import com.example.mohsen.adapter.AdapterDelivery
import com.example.mohsen.databinding.Fragment5Binding
...
class FragmentFive : Fragment() {
    ...
    private val itemAdapter: AdapterDelivery by lazy { AdapterDelivery(items, requireContext()) }
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View {
        binding = Fragment5Binding.inflate(inflater)
        return binding.root
    }
    ...
    private fun loadItemsFromSharedPreferences() {
        val sharedPreferences = requireContext().getSharedPreferences("Delivery", 0)
        ...
        for (i in 0 until jsonArray.length()) {
            ...
            items.add(Item(text1, text2, "Delivery"))
        }
